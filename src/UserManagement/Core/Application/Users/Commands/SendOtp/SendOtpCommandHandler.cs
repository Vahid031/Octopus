...
public record SendOtpCommandHandler : IRequestHandler<SendOtpCommand, DateTimeOffset>
{
	...
	public async Task<DateTimeOffset> Handle(SendOtpCommand request, CancellationToken cancellationToken)
	{
		...
		var otp = user.CreateNewOtpCode(request.IpAddress, _otpOptions.Value.ExpireDuration);
